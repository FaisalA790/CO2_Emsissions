...
if user_input == 'Visulization':
    ...
    fig3 = sns.barplot(data=df_vehicle_class, x="Vehicle Class", y="index")
    plt.xticks(rotation=75)
    ...
    plt.figure(figsize=(20, 6))
    fig4 = sns.barplot(data=df_engine_size, x="Engine Size(L)", y="index")
    ...
    sns.barplot(data=df_fuel_type, x="Fuel Type", y="index")
    ...
    sns.barplot(data=df_ftype, x="Fuel Type", y="index")
    plt.title("All Fuel Types")
    plt.xlabel("Fuel Types")
    plt.ylabel("Cars")
    plt.bar_label(plt.gca().containers[0])
    ...
    cylinders=st.selectbox('Cylinders', df['Cylinders'].unique())
    ...
    st.write(f'Approximate Predicted CO2 Emissions from the car is: {predicted_co2[0]:.2f} g/km')
